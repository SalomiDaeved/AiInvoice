...
export async function createBusinessProfile(req, res) {
    ...
    return res.status(201).json({success: true, data: saved, message:"Business profile created successfully"});  
        ...
        console.err("Error creating business profile:", err);
...
export async function updateBusinessProfile(req, res) {
   ...
   const {userId} = getAuth(req);
    ...
    const updated = await BusinessProfile.findByIdAndUpdate(id, update, { new: true, runValidators: true });
       ...
       return res.status(200).json({success: true, data: updated, message:"Business profile updated successfully"});    


        
    } catch (err) {
        console.err("Error updating business profile:", err);
        return res.status(500).json({success: false, message:"Internal server error"}); 
...
if(!profile){
    return res.status(204).json({success: false, message:"Business profile not found"});
}
return res.status(200).json({success: true, data: profile});
        ...
        console.err("Error fetching business profile:", err);
        return res.status(500).json({success: false, message:"Internal server error"});
