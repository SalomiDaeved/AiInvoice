...
import { kpiCardStyles } from '../assets/Styles.js'
...
const MetricIcons = {
  ...
  // LKR Revenue Icon (LIGHT VERSION)
...
const KpiCard = ({
  ...
  const IconComponent =
    MetricIcons[iconType] || MetricIcons.default;

  const getTrendColor = (trendValue) => {
    if (trendValue > 0)
      return kpiCardStyles.trendBadgePositive;

    if (trendValue < 0)
      return kpiCardStyles.trendBadgeNegative;
  ...
  const getIconColor = (type) => {
    return (
      kpiCardStyles.iconColors[type] ||
      kpiCardStyles.iconColors.default
    );
  };
...
{trend !== undefined && (
    <div  className={`${kpiCardStyles.trendBadge} ${getTrendColor(
        ...
        <div className={kpiCardStyles.textContent}>
            <div className={kpiCardStyles.title}> {title}
